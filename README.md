...
> \* Voice Assistant requires a speech-to-text and intent processing
> pipeline (e.g. Home Assistant Assist). Control4 does not natively
> provide voice intent handling, so this entity type is not supported.
...
`Password`. Sets the device password. This must match the password
...
- Fixed automatic driver updates not working when the leader instance is
...
- Fixed cover contact sensors sending duplicate notifications during
  open/close operations
- Fixed Yale DoorSense contact sensor sending duplicate "Closed"
  notifications on every poll cycle by tracking the last known door
  status and only reporting on actual state changes
